...
function recommendationScore(p){const rotation=Number(p.ventasUltimos30Dias)||0,stock=Number(p.stockDisponible??p.stock),urgency=stock===0?40:stock<p.stockMinimo?25:stock<p.stockIdeal?10:0;return rotation*100+urgency}
...
function openOrders(){
  const html=state.orders.length?state.orders.map(o=>{const received=o.estado==='RECIBIDO';const receivedLabel=o.recibidoAt?`Recibido el ${dateTime.format(new Date(o.recibidoAt))} · Ya está en stock`:'Ya está en stock';return `<article class="history-card"><div class="history-head"><strong>${o.pedidoId}</strong><span>${dateTime.format(new Date(o.fecha))}</span></div><div class="history-items">${o.detalles.map(d=>{const p=state.products.find(x=>x.productoId===d.productoId);return `${p?.emoji||''} ${escapeHtml(p?.nombre||d.productoId)} × ${d.cantidad}<br>`}).join('')}</div><div class="history-total"><span>${o.estado}</span><span>${money.format(o.costoTotal)}</span></div>${received?`<div class="received-note">✅ ${receivedLabel}</div>`:''}<button class="whatsapp-button" data-share-order="${o.pedidoId}">💬 Enviar por WhatsApp</button>${o.estado==='PEDIDO'?`<button class="secondary-button" data-edit-order="${o.pedidoId}">✏️ Modificar pedido</button><button class="secondary-button" data-receive="${o.pedidoId}">Marcar como recibido</button>`:''}</article>`}).join(''):'<div class="empty">Aún no hay pedidos</div>';
...
async function receiveOrder(id){
  if(receivingOrders.has(id))return;
  if(!await appConfirm('¿Confirmas que recibiste todo este pedido?','Recibir pedido'))return;
  receivingOrders.add(id);
  setLoading(true);
  try{await apiRequest('receiveOrder',{pedidoId:id});closeModal();toast('Pedido recibido e inventario actualizado');await refreshAll(false)}catch(e){console.error(e);toast(e.message,true)}finally{receivingOrders.delete(id);setLoading(false)}
}
...
async function demoApi(action,data){await new Promise(r=>setTimeout(r,180));const db=loadDemo();const product=id=>db.products.find(p=>p.productoId===id);const movement=(p,cantidad,tipo,referencia,motivo)=>{const old=p.stock;p.stock+=cantidad;db.movements.push({movimientoId:demoId('M',db.movements,'movimientoId'),fecha:new Date().toISOString(),productoId:p.productoId,tipo,cantidad,stockAnterior:old,stockNuevo:p.stock,referencia,motivo})};
  ...
  if(action==='receiveOrder'){const o=db.orders.find(x=>x.pedidoId===data.pedidoId);if(!o)throw new Error('El pedido no existe');if(o.estado==='RECIBIDO')return{success:true,data:o,message:'Pedido ya recibido'};if(o.estado!=='PEDIDO')throw new Error('El pedido no está pendiente');o.detalles.forEach(d=>movement(product(d.productoId),d.cantidad,'ENTRADA',o.pedidoId,'Pedido recibido'));o.estado='RECIBIDO';o.recibidoAt=new Date().toISOString();saveDemo(db);return{success:true,data:o,message:'Pedido recibido'}}
